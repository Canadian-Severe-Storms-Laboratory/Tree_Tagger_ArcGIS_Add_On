...
try:
    i = 0
    while argv[i] != "?":
        directionGridParameters.append([int(argv[i]), int(argv[i+1])])
        i += 2
    i += 1
    argv = argv[i:]
    ...
    imageScale = (round(float(argv[0]), 6), round(float(argv[1]), 6))
...
for imgData in tqdm(imagesData, file=sys.stdout):
    ...
    try:
        ...
        if imageScale[0] != 0.05 or imageScale[1] != 0.05:
            ...
            image = cv2.resize(image, (int(image.shape[1] * resize_factor[0]), int(image.shape[0] * resize_factor[1])), cv2.INTER_CUBIC)
...
projectPathExtension = "/TreeTagger/Results_" + datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
try:
    os.mkdir(projectPath + projectPathExtension)
    ...
    shpw = shapefile.Writer(projectPath + projectPathExtension + '/Lines_' + datetime.now().strftime("%Y-%m-%d_%H-%M-%S"), shapeType=3)
    shpw.field('ID', 'N')
...
for imgData in tqdm(imagesData, file=sys.stdout):
    ...
    for line in coordLines:
        if (imgData[1] <= line[0] <= (imgData[1] + originalSize[1] * imageScale[0]) and (imgData[2] - originalSize[0] * imageScale[1]) <= line[1] <= imgData[2]) or (imgData[1] <= line[2] <= (imgData[1] + originalSize[1] * imageScale[0]) and (imgData[2] - originalSize[0] * imageScale[1]) <= line[3] <= imgData[2]):
            ...
            newCoordLines.append(line)

    coordLines = newCoordLines
    ...
    imageLineBatchSize = 256
    i = 0
    while i + imageLineBatchSize < len(imageLines):
        imageLineBatches.append(imageLines[i:i+imageLineBatchSize])
        i += imageLineBatchSize
    ...
    for b in tqdm(range(len(imageLineBatches)), file=sys.stdout, leave=False):
        ...
        predictions = model.predict(np.asarray(batchImages), batch_size=imageLineBatchSize)
...
try:
    #converting pixels to coords
    coordLines = [[topLeft[0] + l[0] * imageScale[0], topLeft[1] - l[1] * imageScale[1], topLeft[0] + l[2] * imageScale[0], topLeft[1] - l[3] * imageScale[1]] for l in predicted_lines]

    shpw = shapefile.Writer(projectPath + projectPathExtension + '/Vectors_' + datetime.now().strftime("%Y-%m-%d_%H-%M-%S"), shapeType=3)
    shpw.field('ID', 'N')
...
for i in range(len(directionGridParameters)):
    size = directionGridParameters[i][0]
    minTrees = directionGridParameters[i][1]
    ...
    grid_directions.append(FastLib.averageDirections(predicted_lines, (size/imageScale[0]), x, y, 7, minTrees))

grid_vectors = []

#using the average grid directions, build vectors to be displayed by arcgis
print("Building vectors")
for k in range(len(directionGridParameters)):
    size = directionGridParameters[k][0]
...
try:
    ...
    shpw = shapefile.Writer(projectPath + projectPathExtension + '/Refined_Lines_' + datetime.now().strftime("%Y-%m-%d_%H-%M-%S"), shapeType=3)
    shpw.field('ID', 'N')
...
try:
    ...
    shpw = shapefile.Writer(projectPath + projectPathExtension + '/Directions_' + datetime.now().strftime("%Y-%m-%d_%H-%M-%S"), shapeType=3)
...
try:
    ...
    file = open(projectPath + projectPathExtension + '\\Directions_' + datetime.now().strftime("%Y-%m-%d_%H-%M-%S") + '.csv', 'w', newline='')
...
try:
    ...
    shpw = shapefile.Writer(projectPath + projectPathExtension + '/Regions_' + datetime.now().strftime("%Y-%m-%d_%H-%M-%S"), shapeType=5)
    shpw.field('ID', 'N')
