...
def crop_coord_polygon(coord_points, image_scale, region_scale, top_left, mask):
    ...
    pixel_points = coords_points_to_pixels(coord_points, scale, top_left)
...
def batch_line_boxes(image, batch, extended_region, resize_offset):
    ...
    for i in range(len(batch)):
        ...
        scale = (resize_offset[0] / 2.0) / math.sqrt(1 + nr_slope ** 2)
        ...
        box_image = rotated_image[mid_point[1] - 128:mid_point[1] + 128, mid_point[0] - 256:mid_point[0] + 256]
...
def find_surrounding_images(image, image_data, image_scale, all_images, image_sizes, line_extension_size):
    ...
    for i in range(len(all_images)):
        ...
        if width < line_extension_size * 3 or height < line_extension_size * 3:
            continue
