...
def export_combined_direction_shapefile(grid_vectors, file_path, direction_grid_parameters, direction_interpolation, using_points=False):
    shpw = shapefile.Writer(file_path + "combined_" + direction_interpolation, shapeType=shapefile.POINT if using_points else shapefile.POLYLINE)
    shpw.field('Consistency', 'F', decimal=6)
    shpw.field('Scale', 'N')

    if using_points:
        shpw.field('Angle', 'F', decimal=6)

        for i in range(len(grid_vectors)):

            for l in grid_vectors[i]:
                shpw.point((l[0] + l[2]) / 2.0, (l[1] + l[3]) / 2.0)
                shpw.record(Consistency=l[4], Scale=(i + 1), Angle=math.degrees(math.atan2(l[3] - l[1], l[2] - l[0])))
...
def export_direction_shapefile(grid_vectors, file_path, direction_grid_parameters, direction_interpolation, using_points=False):
    #t = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")

    for i in range(len(grid_vectors)):

        size = direction_grid_parameters[i][0]
        min_trees = direction_grid_parameters[i][1]

        shpw = shapefile.Writer(file_path + str(size) + "m_" + str(min_trees) + "Trees_" + direction_interpolation, shapeType=shapefile.POINT if using_points else shapefile.POLYLINE)
        shpw.field('Consistency', 'F', decimal=6)
        shpw.field('Scale', 'N')
